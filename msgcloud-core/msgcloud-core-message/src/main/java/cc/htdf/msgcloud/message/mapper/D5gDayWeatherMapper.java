...
public interface D5gDayWeatherMapper extends BaseMapper<D5gDayWeatherPO> {
    ...
    @Select("select * from d_5g_day_weather where areaname = #{areaname} and publish_date = (select max(publish_date) from d_5g_day_weather)\n" +
            "and valid_date = (select min(valid_date) from d_5g_day_weather where publish_date = (select max(publish_date) from d_5g_day_weather))")
    D5gDayWeatherPO getLastDataByAreaname(@Param("areaname") String areaname);
