...
public interface D5gObserveWeatherMapper extends BaseMapper<D5gObserveWeatherPO> {
    ...
    @Select("select * from d_5g_observe_weather where area = #{areaname} and publish_date = (select max(publish_date) from d_5g_observe_weather)")
    D5gObserveWeatherPO getLastDataByAreaname(@Param("areaname")  String areaname);
}
